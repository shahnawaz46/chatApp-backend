...
const socketConnection = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("friend_request_send", async (request) => {
      const updatedUser = await upadateNotificationInUserCollection(
        request,
        "$push"
      );

      io.to(allOnlineUser[request.receiverId]).emit(
    ...
    socket.on("remove_friend", async (bothIds) => {
      ...
      io.to(allOnlineUser[bothIds.loginId]).emit("friend_remove", {
        updatedUser: updatedUser.sender,
        key,
      });

      io.to(allOnlineUser[bothIds.removerId]).emit("friend_remove", {
        updatedUser: updatedUser.remover,
        key,
      });
    ...
    socket.on("send_message", async (messageDetail) => {
      const key = [messageDetail.senderId, messageDetail.receiverId]
        .sort()
        .join("-");

      if (allOnlineUser[messageDetail.receiverId])
        io.to(allOnlineUser[messageDetail.receiverId]).emit(
          "receive_message",
          messageDetail
        );
      else {
        io.to(allOnlineUser[messageDetail.senderId]).emit(
          "when_receiver_offline",
          { key, messageDetail }
        );
    ...
    socket.on("store_message", async (messageDetail) => {
      ...
      io.to(allOnlineUser[messageDetail.senderId])
        .to(allOnlineUser[messageDetail.receiverId])
        .emit("store_message_in_client", { key, messageDetail });
    ...
    socket.on("retrieve_message", async (_id) => {
      ...
      Object.keys(messagesObj).length > 0 &&
        io
          .to(allOnlineUser[_id])
          .emit("retrieve_message_client", { messagesObj });
    });

    socket.on("messages_seen", async ({ key, messages }) => {
      const [id_1, id_2] = key.split("-");

      io.to(allOnlineUser[id_1])
        .to(allOnlineUser[id_2])
        .emit("message_seen_client", { key, messages });
